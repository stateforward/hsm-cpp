...
TEST_CASE("Deferral - Basic") {
  constexpr auto model =
      define("machine", initial(target("idle")),
             state("idle", defer("event_A"),
                   transition(on("event_B"), target("processing"))),
             state("processing", transition(on("event_A"), target("done"))),
             state("done"));
...
TEST_CASE("Deferral - Hierarchy") {
  ...
  // SCXML: <state id="p"><onentry>...</onentry><transition event="e"
  // .../></state> If child doesn't handle e, parent handles it. If parent
  // defers e, child inherits deferral? cthsm implementation checks parent
  // deferral list. My `is_deferred` implementation iterates hierarchy.

  constexpr auto model = define(
      "machine", initial(target("p/c")),
      state("p", defer("event_A"),
            state("c", transition(on("event_B"), target("/machine/other")))),
      state("other", transition(on("event_A"), target("done"))), state("done"));
