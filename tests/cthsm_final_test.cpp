...
// Action functions
void log_entry_start(Context&, Instance& i, const Event&) { static_cast<FinalStatesInstance&>(i).log("entry_start"); }
void log_entry_active(Context&, Instance& i, const Event&) { static_cast<FinalStatesInstance&>(i).log("entry_active"); }
void log_entry_container(Context&, Instance& i, const Event&) { static_cast<FinalStatesInstance&>(i).log("entry_container"); }
void log_entry_working(Context&, Instance& i, const Event&) { static_cast<FinalStatesInstance&>(i).log("entry_working"); }
void log_entry_reset(Context&, Instance& i, const Event&) { static_cast<FinalStatesInstance&>(i).log("entry_reset"); }
void log_entry_level1(Context&, Instance& i, const Event&) { static_cast<FinalStatesInstance&>(i).log("entry_level1"); }
void log_entry_level2(Context&, Instance& i, const Event&) { static_cast<FinalStatesInstance&>(i).log("entry_level2"); }
void log_entry_step1(Context&, Instance& i, const Event&) { static_cast<FinalStatesInstance&>(i).log("entry_step1"); }
void log_entry_step2(Context&, Instance& i, const Event&) { static_cast<FinalStatesInstance&>(i).log("entry_step2"); }

TEST_CASE("Final States - Basic Functionality") {
    SUBCASE("Simple Final State") {
        constexpr auto model = define(
            "SimpleFinal",
            initial(target("/SimpleFinal/start")),
            state("start",
                entry(log_entry_start),
    ...
    SUBCASE("Final State with Multiple Paths") {
        constexpr auto model = define(
            "MultipleFinal",
            initial(target("/MultipleFinal/active")),
            state("active",
                entry(log_entry_active),
...
TEST_CASE("Final States - Semantic Restrictions") {
    SUBCASE("Final State Cannot Have Entry Actions") {
        // In cthsm, final() does not accept entry actions, so we can't even compile if we tried.
        // We just verify basic behavior.
        constexpr auto model = define(
            "FinalWithEntry",
            initial(target("/FinalWithEntry/start")),
            state("start",
    ...
    // Skipping "Cannot Have Exit Actions" and "Cannot Have Outgoing Transitions" as they are enforced by API
    ...
    SUBCASE("Final State Cannot Have Outgoing Transitions (Runtime Check)") {
        ...
        sm.dispatch(instance, "RESTART");
        CHECK(sm.state() == "/FinalNoTransitions/end");
        
        sm.dispatch(instance, "CONTINUE");
        CHECK(sm.state() == "/FinalNoTransitions/end");
        
        sm.dispatch(instance, "ANY_EVENT");
        CHECK(sm.state() == "/FinalNoTransitions/end");
    }
}
...
TEST_CASE("Final States - Hierarchical Scenarios") {
    SUBCASE("Final State in Nested State") {
        constexpr auto model = define(
            "NestedFinal",
            initial(target("/NestedFinal/container/working")), // Direct target to working
            state("container",
                entry(log_entry_container),
                // initial(target("working")), // Not using local initial for simplicity/cthsm preference
                state("working",
                    entry(log_entry_working),
                    transition(on("COMPLETE"), target("/NestedFinal/container/done"))
                ),
                final("done"),
                transition(on("RESET"), target("/NestedFinal/reset"))
            ),
            state("reset",
                entry(log_entry_reset)
            )
        );
    ...
    SUBCASE("Multiple Final States in Hierarchy") {
        constexpr auto model = define(
            ...
            state("level1",
                entry(log_entry_level1),
                // initial(target("level2")),
                state("level2",
                    entry(log_entry_level2),
                    transition(on("FINISH_INNER"), target("/HierarchicalFinals/level1/inner_done"))
                ),
                final("inner_done"),
                transition(on("FINISH_OUTER"), target("/HierarchicalFinals/outer_done"))
            ),
            final("outer_done")
        );
...
TEST_CASE("Final States - Error Conditions and Edge Cases") {
    SUBCASE("Final State as Initial Target") {
        constexpr auto model = define(
            "ImmediateFinal",
            initial(target("/ImmediateFinal/end")),
            final("end")
        );
    ...
    SUBCASE("Final State with Deferred Events") {
        ...
        // (Actually cthsm supports defer() on final() ?? No, final() takes only Name)
        // So this test is checking behavior if we just send event.
        constexpr auto model = define(
            "FinalWithDeferred",
            initial(target("/FinalWithDeferred/start")),
            state("start",
        ...
        sm.dispatch(instance, "DEFERRED_EVENT");
        CHECK(sm.state() == "/FinalWithDeferred/end");
    }
    ...
    SUBCASE("Rapid Transitions to Final State") {
        ...
        sm.dispatch(instance, "STEP1");
        CHECK(sm.state() == "/RapidFinal/step1");
        
        sm.dispatch(instance, "STEP2");
        CHECK(sm.state() == "/RapidFinal/step2");
        
        sm.dispatch(instance, "FINAL");
