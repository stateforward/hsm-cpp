...
TEST_CASE("Choice States - Effects on Transitions") {
  SUBCASE("Choice Transitions with Effects") {
    constexpr auto model =
        define("ChoiceWithEffects", initial(target("/ChoiceWithEffects/start")),
               state("start",
  ...
  SUBCASE("Choice Inside Nested State") {
    constexpr auto model = define(
        ...
        state("container", entry(entry_container),
              // initial(target("/NestedChoice/container/start")), // cthsm:
...
TEST_CASE("Choice States - Error Conditions and Edge Cases") {
  SUBCASE("Choice State Cannot Be Initial Target") {
    constexpr auto model =
        define("InitialChoice", initial(target("/InitialChoice/choice")),
               choice("choice", transition(guard(guard_always_true),
                                           target("/InitialChoice/target"))),
