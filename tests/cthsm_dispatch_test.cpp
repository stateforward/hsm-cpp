...
TEST_CASE("Dispatch - Hierarchical Transition") {
    constexpr auto model = define("machine",
        initial(target("idle")),
        state("idle",
            transition(on("start"), target("working"))
        ),
        state("working",
...
TEST_CASE("Dispatch - Sibling Transitions") {
    constexpr auto model = define("machine",
        initial(target("s1")),
        state("s1",
            transition(on("next"), target("s2"))
        ),
        state("s2",
            transition(on("next"), target("s3"))
        ),
        state("s3",
