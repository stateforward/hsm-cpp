...
struct TimerInstance : public Instance {
    ...
    bool has_timer(std::size_t id) const {
        for (const auto& t : active_timers) if (t.id == id) return true;
        return false;
    }
};
...
TEST_CASE("Timers - After") {
    constexpr auto model = define("machine",
        initial(target("idle")),
        state("idle",
            transition(after(timer_100ms), target("timeout"))
        ),
        state("timeout")
    );
    ...
    std::size_t tid = inst.active_timers[0].id;
    
    // Fire timer
    sm.handle_timer(inst, tid);
    CHECK(sm.state() == "/machine/timeout");
    
    // Timer should be cancelled on exit (if single shot? implementation detail of cthsm doesn't auto cancel unless 'every'?)
    // 'after' is transition trigger. Transition taken -> exit state.
    // Exit state logic cancels timers of that state.
    // So timer should be cancelled.
...
TEST_CASE("Timers - Cancel on External Transition") {
    constexpr auto model = define("machine",
        initial(target("idle")),
        state("idle",
            transition(after(timer_200ms), target("timeout")),
