...
// Behaviors
void entry_composite(Context&, Instance& i, const Event&) { 
    static_cast<CompositeInstance&>(i).log("entry_composite"); 
    static_cast<CompositeInstance&>(i).entry_count++;
}
void exit_composite(Context&, Instance& i, const Event&) { 
    static_cast<CompositeInstance&>(i).log("exit_composite"); 
    static_cast<CompositeInstance&>(i).exit_count++;
}

void entry_child1(Context&, Instance& i, const Event&) { 
    static_cast<CompositeInstance&>(i).log("entry_child1"); 
    static_cast<CompositeInstance&>(i).entry_count++;
}
void exit_child1(Context&, Instance& i, const Event&) { 
    static_cast<CompositeInstance&>(i).log("exit_child1"); 
    static_cast<CompositeInstance&>(i).exit_count++;
}

void entry_child2(Context&, Instance& i, const Event&) { 
    static_cast<CompositeInstance&>(i).log("entry_child2"); 
    static_cast<CompositeInstance&>(i).entry_count++;
}
void exit_child2(Context&, Instance& i, const Event&) { 
    static_cast<CompositeInstance&>(i).log("exit_child2"); 
    static_cast<CompositeInstance&>(i).exit_count++;
}

void entry_other(Context&, Instance& i, const Event&) { 
    static_cast<CompositeInstance&>(i).log("entry_other"); 
    static_cast<CompositeInstance&>(i).entry_count++;
}

void effect_initial(Context&, Instance& i, const Event&) { 
    static_cast<CompositeInstance&>(i).log("effect_initial"); 
    static_cast<CompositeInstance&>(i).effect_count++;
}

void entry_outer(Context&, Instance& i, const Event&) { static_cast<CompositeInstance&>(i).log("entry_outer"); static_cast<CompositeInstance&>(i).entry_count++; }
void entry_inner(Context&, Instance& i, const Event&) { static_cast<CompositeInstance&>(i).log("entry_inner"); static_cast<CompositeInstance&>(i).entry_count++; }
void entry_deepest(Context&, Instance& i, const Event&) { static_cast<CompositeInstance&>(i).log("entry_deepest"); static_cast<CompositeInstance&>(i).entry_count++; }
void entry_deepest_alt(Context&, Instance& i, const Event&) { static_cast<CompositeInstance&>(i).log("entry_deepest_alt"); static_cast<CompositeInstance&>(i).entry_count++; }
void entry_inner_alt(Context&, Instance& i, const Event&) { static_cast<CompositeInstance&>(i).log("entry_inner_alt"); static_cast<CompositeInstance&>(i).entry_count++; }

void entry_start(Context&, Instance& i, const Event&) { static_cast<CompositeInstance&>(i).log("entry_start"); static_cast<CompositeInstance&>(i).entry_count++; }
void exit_start(Context&, Instance& i, const Event&) { static_cast<CompositeInstance&>(i).log("exit_start"); static_cast<CompositeInstance&>(i).exit_count++; }

void entry_comp1(Context&, Instance& i, const Event&) { static_cast<CompositeInstance&>(i).log("entry_comp1"); static_cast<CompositeInstance&>(i).entry_count++; }
void entry_comp1_child1(Context&, Instance& i, const Event&) { static_cast<CompositeInstance&>(i).log("entry_comp1_child1"); static_cast<CompositeInstance&>(i).entry_count++; }
void entry_comp1_child2(Context&, Instance& i, const Event&) { static_cast<CompositeInstance&>(i).log("entry_comp1_child2"); static_cast<CompositeInstance&>(i).entry_count++; }

void entry_comp2(Context&, Instance& i, const Event&) { static_cast<CompositeInstance&>(i).log("entry_comp2"); static_cast<CompositeInstance&>(i).entry_count++; }
void entry_comp2_child1(Context&, Instance& i, const Event&) { static_cast<CompositeInstance&>(i).log("entry_comp2_child1"); static_cast<CompositeInstance&>(i).entry_count++; }
void entry_comp2_child2(Context&, Instance& i, const Event&) { static_cast<CompositeInstance&>(i).log("entry_comp2_child2"); static_cast<CompositeInstance&>(i).entry_count++; }
...
TEST_CASE("Composite States with Initial Pseudostates") {
    ...
    SUBCASE("Composite State with Initial and Effect") {
        constexpr auto model = define(
            ...
            state("composite",
                entry(entry_composite),
                initial(target("/CompositeInitialWithEffect/composite/child1"), effect(effect_initial)),
    ...
    SUBCASE("Nested Composite States with Initial") {
        constexpr auto model = define(
            "NestedComposites",
            initial(target("/NestedComposites/outer")),
            state("outer",
                entry(entry_outer),
    ...
    SUBCASE("Transition to Composite State Triggers Initial") {
        constexpr auto model = define(
            ...
            state("start",
                entry(entry_start),
                exit(exit_start),
                transition(on("GO_TO_COMPOSITE"), target("/TransitionToComposite/composite"))
            ),
            state("composite",
                entry(entry_composite),
        ...
        sm.dispatch(instance, "GO_TO_COMPOSITE");
    ...
    SUBCASE("Direct Transition to Nested State Bypasses Initial") {
        constexpr auto model = define(
            "DirectToNested",
            initial(target("/DirectToNested/start")),
            state("start",
                entry(entry_start),
                exit(exit_start),
                transition(on("GO_TO_CHILD2"), target("/DirectToNested/composite/child2"))
            ),
            state("composite",
                entry(entry_composite),
                initial(target("/DirectToNested/composite/child1")),  // This should be bypassed
                state("child1", entry(entry_child1)),
                state("child2", entry(entry_child2))
            )
        );
        ...
        sm.dispatch(instance, "GO_TO_CHILD2");
    ...
    SUBCASE("Multiple Composite States with Different Initials") {
        constexpr auto model = define(
            "MultipleComposites",
            initial(target("/MultipleComposites/comp1")),
            state("comp1",
                entry(entry_comp1),
            ...
            state("comp2",
                entry(entry_comp2),
                initial(target("/MultipleComposites/comp2/comp2_child2")),  // Different initial child
