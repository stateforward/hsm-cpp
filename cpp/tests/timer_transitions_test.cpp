...
std::chrono::milliseconds short_duration(hsm::Context& /*ctx*/, TimerTestInstance& /*inst*/, hsm::Event& /*event*/) {
    return std::chrono::milliseconds(50);
}

std::chrono::milliseconds medium_duration(hsm::Context& /*ctx*/, TimerTestInstance& /*inst*/, hsm::Event& /*event*/) {
    return std::chrono::milliseconds(100);
}

std::chrono::milliseconds zero_duration(hsm::Context& /*ctx*/, TimerTestInstance& /*inst*/, hsm::Event& /*event*/) {
...
void timer_effect(hsm::Context& /*ctx*/, hsm::Instance& inst, hsm::Event& /*event*/) {
    auto& test_inst = static_cast<TimerTestInstance&>(inst);
    test_inst.log("timer_effect");
    test_inst.timer_fired = true;
}

void count_timer(hsm::Context& /*ctx*/, hsm::Instance& inst, hsm::Event& /*event*/) {
    auto& test_inst = static_cast<TimerTestInstance&>(inst);
    ...
    SUBCASE("Basic After Timer") {
        auto model = hsm::define(
            ...
            hsm::state("waiting",
                ...
                hsm::transition(hsm::after<std::chrono::milliseconds, TimerTestInstance>(short_duration), 
                               hsm::target("../done"),
                               hsm::effect(timer_effect))
            ),
            hsm::state("done",
                hsm::entry([](hsm::Context& ctx, hsm::Instance& inst, hsm::Event& event) {
    ...
    SUBCASE("Zero Duration After Timer") {
        auto model = hsm::define(
            ...
            hsm::state("start",
                ...
                hsm::transition(hsm::after<std::chrono::milliseconds, TimerTestInstance>(zero_duration),
    ...
    SUBCASE("After Timer with State Exit Cancellation") {
        auto model = hsm::define(
            ...
            hsm::state("waiting",
                ...
                hsm::transition(hsm::after<std::chrono::milliseconds, TimerTestInstance>(medium_duration),
                               hsm::target("../timeout"),
                               hsm::effect(timer_effect))
            ),
            hsm::state("cancelled",
...
TEST_CASE("Timer Transitions - Every Functionality") {
    SUBCASE("Basic Every Timer") {
        auto model = hsm::define(
            "EveryTimer",
            hsm::initial(hsm::target("repeating")),
            hsm::state("repeating",
                hsm::entry([](hsm::Context& ctx, hsm::Instance& inst, hsm::Event& event) {
                    log_entry("repeating", ctx, inst, event);
                }),
                hsm::transition(hsm::on("STOP"), hsm::target("../stopped")),
                hsm::transition(hsm::every<std::chrono::milliseconds, TimerTestInstance>(short_duration),
    ...
    SUBCASE("Every Timer with Zero Duration") {
        auto model = hsm::define(
            ...
            hsm::state("start",
                ...
                hsm::transition(hsm::every<std::chrono::milliseconds, TimerTestInstance>(zero_duration),
                               hsm::effect(count_timer))
            )
        );
...
TEST_CASE("Timer Transitions - Multiple Timers") {
    SUBCASE("Multiple After Timers in Same State") {
        ...
        CHECK(instance.state() == "/MultipleAfterTimers/fast");
    ...
    SUBCASE("Mixed After and Every Timers") {
        ...
        CHECK(instance.state() == "/MixedTimers/timeout");
    }
}

TEST_CASE("Timer Transitions - Error Conditions") {
    SUBCASE("Timer in Final State") {
        // Final states should not have timers, but this tests the system's robustness
        auto model = hsm::define(
            "TimerInFinal",
            hsm::initial(hsm::target("start")),
            hsm::state("start",
