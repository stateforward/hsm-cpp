...
void set_flag_a(hsm::Context& /*ctx*/, hsm::Instance& inst, hsm::Event& /*event*/) {
    auto& test_inst = static_cast<GuardConditionsInstance&>(inst);
    test_inst.flag_a = true;
    test_inst.log("set_flag_a");
}

void record_event_name(hsm::Context& /*ctx*/, hsm::Instance& inst, hsm::Event& event) {
    auto& test_inst = static_cast<GuardConditionsInstance&>(inst);
...
// Guard functions
bool guard_counter_positive(hsm::Context& /*ctx*/, GuardConditionsInstance& inst, hsm::Event& /*event*/) {
    return inst.counter.load() > 0;
}

bool guard_counter_even(hsm::Context& /*ctx*/, GuardConditionsInstance& inst, hsm::Event& /*event*/) {
    return (inst.counter.load() % 2) == 0;
}

bool guard_counter_greater_than_5(hsm::Context& /*ctx*/, GuardConditionsInstance& inst, hsm::Event& /*event*/) {
    return inst.counter.load() > 5;
}

bool guard_flag_a(hsm::Context& /*ctx*/, GuardConditionsInstance& inst, hsm::Event& /*event*/) {
    return inst.flag_a.load();
}

bool guard_flag_b(hsm::Context& /*ctx*/, GuardConditionsInstance& inst, hsm::Event& /*event*/) {
    return inst.flag_b.load();
}

bool guard_can_transition(hsm::Context& /*ctx*/, GuardConditionsInstance& inst, hsm::Event& /*event*/) {
    return inst.can_transition.load();
}

bool guard_always_true(hsm::Context& /*ctx*/, GuardConditionsInstance& /*inst*/, hsm::Event& /*event*/) {
    return true;
}

bool guard_always_false(hsm::Context& /*ctx*/, GuardConditionsInstance& /*inst*/, hsm::Event& /*event*/) {
    return false;
}

bool guard_event_name_contains_x(hsm::Context& /*ctx*/, GuardConditionsInstance& /*inst*/, hsm::Event& event) {
    return event.name.find('X') != std::string::npos;
}

bool guard_complex_condition(hsm::Context& /*ctx*/, GuardConditionsInstance& inst, hsm::Event& /*event*/) {
    return inst.counter.load() > 2 && inst.flag_a.load() && !inst.flag_b.load();
}

// Stateful guard that changes behavior on each call
bool guard_stateful_toggle(hsm::Context& /*ctx*/, GuardConditionsInstance& inst, hsm::Event& /*event*/) {
...
TEST_CASE("Guard Conditions - Basic Functionality") {
    SUBCASE("Simple Boolean Guard") {
        auto model = hsm::define(
            ...
            hsm::state("start",
                ...
                hsm::transition(hsm::on("ATTEMPT"), hsm::guard(guard_flag_a), hsm::target("../guarded")),
    ...
    SUBCASE("Multiple Guards on Same Event") {
        auto model = hsm::define(
            ...
            hsm::initial(hsm::target("start")),
            hsm::state("start",
                hsm::transition(hsm::on("GO"), hsm::guard(guard_counter_greater_than_5), hsm::target("../high")),
                hsm::transition(hsm::on("GO"), hsm::guard(guard_counter_positive), hsm::target("../low")),
    ...
    SUBCASE("Guard Based on Event Content") {
        auto model = hsm::define(
            ...
            hsm::state("start",
                hsm::transition(
                    hsm::on("PROCESS"),
                    hsm::guard(guard_event_name_contains_x),
                    hsm::target("../special"),
                    hsm::effect(record_event_name)
                ),
            ...
            hsm::state("start",
                hsm::transition(
                    ...
                    hsm::guard([](hsm::Context& ctx, hsm::Instance& /*inst*/, hsm::Event& /*event*/) {
                        // This is a contrived example - in practice, context would have useful state
                        return !ctx.is_set();  // Guard passes if context is not cancelled
                    }),
                    hsm::target("../allowed")
                ),
    ...
    SUBCASE("Nested Guards with Effects") {
        auto model = hsm::define(
            ...
            hsm::state("start",
                hsm::transition(
                    hsm::on("COMPLEX"),
                    hsm::guard(guard_complex_condition),
                ...
                hsm::transition(
                    hsm::on("COMPLEX"),
                    hsm::guard(guard_counter_positive),
                    hsm::target("../simple_target"),
                    hsm::effect([](hsm::Context& ctx, hsm::Instance& inst, hsm::Event& event) {
                        log_effect("simple_effect", ctx, inst, event);
                    })
                ),
                hsm::transition(hsm::on("COMPLEX"), hsm::target("../default_target"))
            ),
...
TEST_CASE("Guard Conditions - Edge Cases and Error Conditions") {
    SUBCASE("Guards with Side Effects") {
        auto model = hsm::define(
            ...
            hsm::state("start",
                ...
                hsm::transition(hsm::on("TEST"), hsm::target("../unguarded"))
            ),
            hsm::state("guarded",
                hsm::entry([](hsm::Context& ctx, hsm::Instance& inst, hsm::Event& event) {
                    log_entry("guarded", ctx, inst, event);
        ...
        instance.dispatch(hsm::Event("TEST")).wait();
        CHECK(instance.state() == "/GuardSideEffects/unguarded");
    ...
    SUBCASE("Guards Always True/False") {
        auto model = hsm::define(
            ...
            hsm::initial(hsm::target("start")),
            hsm::state("start",
                hsm::transition(hsm::on("ALWAYS_TRUE"), hsm::guard(guard_always_true), hsm::target("../always_true_target")),
                hsm::transition(hsm::on("ALWAYS_FALSE"), hsm::guard(guard_always_false), hsm::target("../never_reached")),
                hsm::transition(hsm::on("ALWAYS_FALSE"), hsm::target("../fallback"))
            ),
    ...
    SUBCASE("Rapid Guard Evaluations") {
        ...
        CHECK(instance.counter.load() == 20);  // 2 increments per round
    }
}